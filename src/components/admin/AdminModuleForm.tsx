...
} from "@/actions/admin/module";

import { adminCoursePath } from "@/lib/admin/adminRevalidate";

const initial: ModuleActionState = { ok: true };
...
type Props =
  | { mode: "create"; courseId: string; defaultValues?: undefined }
  | { mode: "edit"; courseId: string; moduleId: string; defaultValues: DefaultValues };

export function AdminModuleForm(props: Props) {
  const action =
    props.mode === "edit"
      ? updateModuleAction.bind(null, props.courseId, props.moduleId)
      : createModuleAction.bind(null, props.courseId);

  const [state, formAction] = useFormState(action, initial);
  const d = props.mode === "edit" ? props.defaultValues : null;
  const cancelHref = adminCoursePath(props.courseId);
  ...
  return (
    <form action={formAction} className="max-w-xl space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-2">
        <SubmitButton label={props.mode === "create" ? "Criar módulo" : "Guardar módulo"} />
        <Link
          href={cancelHref}
